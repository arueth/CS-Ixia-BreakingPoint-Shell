...
class BPS:
    ...
    def login(self, enableRequestPrints=False):
        ...
        if (r.status_code == 200):
            ...
            raise BPS.LoginException(r.content.json().get('error'), r.status_code)
    ...
    def runTest(self, modelname, group, neighborhood=None, enableRequestPrints=False):
        ...
        if (r.status_code == 200):
            ...
            raise BPS.TestException(r.json().get('error'), r.status_code)

    # Stop Test
    def stopTest(self, testid, enableRequestPrints=False):
    ...
    def setMulticast(self, template, enableRequestPrints=False):
        ...
        if (enableRequestPrints):
            self.pretty_print_requests(r)
        if (r.status_code == 204):
            print 'Multicast model: ' + template + ' set successfully for viewing and modification'
    ...
    class LoginException(Exception):
        def __init__(self, message, status_code):
            super(BPS.LoginException, self).__init__(message)
            self.status_code = status_code

    class TestException(Exception):
        def __init__(self, message, status_code):
            super(BPS.TestException, self).__init__(message)
            self.status_code = status_code
